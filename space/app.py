...
with gr.Blocks(title="Context Breach Demo", theme=gr.themes.Soft()) as demo:
    gr.Markdown(
        ...
        "| 🟡 Legacy hackathon checkpoint (not corrected holdout) | **80 reported** |\n"
        ...
        "New checkpoints train only on these scenarios. "
        ...
        "These are excluded from new training runs and are the default evaluation split. "
        "Naive and guarded policies run live; the trained policy fallback shows the guarded containment as "
        "the expected pattern.\n\n"
        "_The legacy checkpoint predates this corrected split and must be retrained before held-out claims are made. "
        "Space runs on free CPU; trained-policy views use cached evaluation traces._"
    )
    with gr.Row():
